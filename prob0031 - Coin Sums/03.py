...
Direct approach - memoization

DEBUGGING IN PROGRESS - fails unit test
'''
...
def getCoins(limit, currentCoins):
    if (limit, currentCoins) in memo:
        return memo[(limit, currentCoins)]
    origCoins = currentCoins
    if limit == 0:
        retval =  []
    elif currentCoins == (1,):
        retval = [{1 : limit}]
