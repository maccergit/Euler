...
Use closed form of sum of even fibonacci numbers.  Use gmpy2 to handle limits over 10**17.
However, gmpy2 is still fixed precision.
'''
...
def solution(limit):
    ...
    phi = (1 + root5) / 2
    upsilon = (1 - root5) / 2
    exp = 3 * int((gmpy2.log10(limit) + gmpy2.log10(5) / 2) / gmpy2.log10(phi) / 3) + 2
    return gmpy2.rint((phi ** exp - upsilon ** exp - root5) / (2 * root5))
